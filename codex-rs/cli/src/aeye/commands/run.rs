...
pub async fn run(cmd: RunCommand, policy_engine: &PolicyEngine) -> Result<()> {
    ...
    let mut engine = WorkflowEngine::new(repo_root, policy_engine);

    let goal = if cmd.recipe == "fix_from_logs" {
        let log_path = cmd
            .log_file
            .context("The `fix_from_logs` recipe requires a --log-file argument.")?;
        let log_content = fs::read_to_string(&log_path)?;
        let truncated_log = log_content.chars().take(1000).collect::<String>();
        Some(format!(
            "Fix the error found in the following logs from {}:\n\n{}",
            log_path.display(),
            truncated_log
        ))
    } else {
