...
use crate::login::run_login_with_chatgpt;
...
#[clap(
    ...
    // `a-eye-x86_64-unknown-linux-musl`, but the help output should always use
    // the generic `a-eye` command name that users run.
    bin_name = "a-eye",
    override_usage = "a-eye [OPTIONS] [PROMPT]\n       a-eye [OPTIONS] <COMMAND> [ARGS]"
)]
struct MultitoolCli {
    #[clap(flatten)]
...
enum Subcommand {
    ...
    /// [experimental] Run A-Eye as an MCP server and manage MCP servers.
    Mcp(McpCli),

    /// [experimental] Run the A-Eye MCP server (stdio transport).
    McpServer,
    ...
    /// Run commands within an A-Eye-provided sandbox.
    ...
    /// Apply the latest diff produced by A-Eye agent as a `git apply` to your local working tree.
    #[clap(visible_alias = "a")]
    Apply(ApplyCommand),

    /// A-Eye: Intent-driven NLPg development agent.
    #[clap(name = "a-eye", visible_alias = "aeye")]
    AEye(aeye::AEyeCli),
    ...
    /// [EXPERIMENTAL] Browse tasks from A-Eye Cloud and apply changes locally.
...
struct LoginCommand {
    ...
    #[arg(
        long = "with-api-key",
        help = "Read the API key from stdin (e.g. `printenv OPENAI_API_KEY | a-eye login --with-api-key`)"
    )]
    with_api_key: bool,
...
fn format_exit_messages(exit_info: AppExitInfo, color_enabled: bool) -> Vec<String> {
    ...
    {
        let resume_cmd = if let Some(rest) = resume_cmd.strip_prefix("codex ") {
            format!("a-eye {rest}")
        } else {
            resume_cmd
        };
...
fn run_update_action(action: UpdateAction) -> anyhow::Result<()> {
    ...
    println!("Updating A-Eye via `{cmd_str}`...");
    ...
    println!("\n🎉 Update ran successfully! Please restart A-Eye.");
    Ok(())
}
...
pub fn run_main() -> anyhow::Result<()> {
    arg0_dispatch_or_else(|codex_linux_sandbox_exe| async move {
        run_with_sandbox_exe(codex_linux_sandbox_exe).await?;
        Ok(())
    })
}

pub async fn run_with_sandbox_exe(codex_linux_sandbox_exe: Option<PathBuf>) -> anyhow::Result<()> {
...
async fn cli_main(codex_linux_sandbox_exe: Option<PathBuf>) -> anyhow::Result<()> {
    ...
    match subcommand {
        ...
        Some(Subcommand::Review(review_args)) => {
            let mut exec_cli = ExecCli::try_parse_from(["a-eye", "exec"])?;
            ...
            match login_cli.action {
                ...
                None => {
                    if login_cli.use_device_code {
                        ...
                        eprintln!(
                            "The --api-key flag is no longer supported. Pipe the key instead, e.g. `printenv OPENAI_API_KEY | a-eye login --with-api-key`."
                        );
                        std::process::exit(1);
        ...
        Some(Subcommand::AEye(mut aeye_cli)) => {
            prepend_config_flags(
                &mut aeye_cli.config_overrides,
                root_config_overrides.clone(),
            );
            aeye::run_main(aeye_cli, codex_linux_sandbox_exe).await?;
        }
...
async fn run_interactive_tui(
    ...
    if terminal_info.name == TerminalName::Dumb {
        ...
        eprintln!(
            "WARNING: TERM is set to \"dumb\". A-Eye's interactive TUI may not work in this terminal."
        );
...
/// Build the final `TuiCli` for an `a-eye resume` invocation.
fn finalize_resume_interactive(
    ...
    // configuration surface area as `a-eye` without additional flags.
...
/// Build the final `TuiCli` for an `a-eye fork` invocation.
...
/// Merge flags provided to `a-eye resume`/`a-eye fork` so they take precedence over any
...
mod tests {
    ...
    fn exec_resume_last_accepts_prompt_positional() {
        let cli =
            MultitoolCli::try_parse_from(["a-eye", "exec", "--json", "resume", "--last", "2+2"])
    ...
    fn format_exit_messages_includes_resume_hint_without_color() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "To continue this session, run a-eye resume 123e4567-e89b-12d3-a456-426614174000"
                    .to_string(),
            ]
        );
    }
    ...
    fn format_exit_messages_prefers_thread_name() {
        ...
        assert_eq!(
            ...
            vec![
                "Token usage: total=2 input=0 output=2".to_string(),
                "To continue this session, run a-eye resume my-thread".to_string(),
            ]
        );
    }
    ...
    fn resume_model_flag_applies_when_no_root_flags() {
        let interactive =
            finalize_resume_from_args(["a-eye", "resume", "-m", "gpt-5.1-test"].as_ref());
    ...
    fn resume_picker_logic_none_and_not_last() {
        let interactive = finalize_resume_from_args(["a-eye", "resume"].as_ref());
        assert!(interactive.resume_picker);
        assert!(!interactive.resume_last);
        assert_eq!(interactive.resume_session_id, None);
    ...
    fn resume_picker_logic_last() {
        let interactive = finalize_resume_from_args(["a-eye", "resume", "--last"].as_ref());
        assert!(!interactive.resume_picker);
        assert!(interactive.resume_last);
        assert_eq!(interactive.resume_session_id, None);
        assert!(!interactive.resume_show_all);
    ...
    fn resume_picker_logic_with_session_id() {
        let interactive = finalize_resume_from_args(["a-eye", "resume", "1234"].as_ref());
        assert!(!interactive.resume_picker);
        assert!(!interactive.resume_last);
    ...
    fn resume_all_flag_sets_show_all() {
        let interactive = finalize_resume_from_args(["a-eye", "resume", "--all"].as_ref());
        assert!(interactive.resume_picker);
        assert!(interactive.resume_show_all);
    }
    ...
    fn fork_picker_logic_none_and_not_last() {
        let interactive = finalize_fork_from_args(["a-eye", "fork"].as_ref());
    ...
    fn fork_picker_logic_last() {
        let interactive = finalize_fork_from_args(["a-eye", "fork", "--last"].as_ref());
        assert!(!interactive.fork_picker);
        assert!(interactive.fork_last);
        assert_eq!(interactive.fork_session_id, None);
        assert!(!interactive.fork_show_all);
    ...
    fn fork_picker_logic_with_session_id() {
        let interactive = finalize_fork_from_args(["a-eye", "fork", "1234"].as_ref());
        assert!(!interactive.fork_picker);
        assert!(!interactive.fork_last);
    ...
    fn fork_all_flag_sets_show_all() {
        let interactive = finalize_fork_from_args(["a-eye", "fork", "--all"].as_ref());
        assert!(interactive.fork_picker);
        assert!(interactive.fork_show_all);
    }
    ...
    fn app_server_analytics_default_disabled_without_flag() {
        let app_server = app_server_from_args(["a-eye", "app-server"].as_ref());
    ...
    fn app_server_analytics_default_enabled_with_flag() {
        let app_server =
            app_server_from_args(["a-eye", "app-server", "--analytics-default-enabled"].as_ref());
        assert!(app_server.analytics_default_enabled);
    }

    #[test]
    fn features_enable_parses_feature_name() {
        let cli = MultitoolCli::try_parse_from(["a-eye", "features", "enable", "unified_exec"])
            .expect("parse should succeed");
    ...
    fn features_disable_parses_feature_name() {
        let cli = MultitoolCli::try_parse_from(["a-eye", "features", "disable", "shell_tool"])
            .expect("parse should succeed");
        let Some(Subcommand::Features(FeaturesCli { sub })) = cli.subcommand else {
            panic!("expected features subcommand");
        };
